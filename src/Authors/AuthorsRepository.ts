...
export class AuthorsRepository {
    ...
    load = async () => {
        const responseDto = await this._apiGateway.get(`/authors?emailOwnerId=${this._userModel.email}`)
    ...
    addAuthor = async (name: string) => {
        ...
        for (const book of this._booksRepository.booksProgrammerModel) {
            await this._booksRepository.add(book.name)
            if (this._booksRepository.lastAddedBook !== null) {
                bookIds.push(this._booksRepository.lastAddedBook.bookId)
            }
        }
